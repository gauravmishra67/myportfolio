...
function OrbitingSatellites() {
  ...
  const satellites = [
    { label: "UI/UX & Frontend Developer", angle: 0, radius: 2.3, yOffset: 0.8 },
    { label: "Creative Writer", angle: (2 * Math.PI) / 3, radius: 2.5, yOffset: -0.2 },
    { label: "AI Learner", angle: (4 * Math.PI) / 3, radius: 2.3, yOffset: -0.8 },
  ];
...
export default function CubeHero() {
  ...
  if (!mounted || error) return <CubeFallback />;
    ...
    <div className="w-full h-full min-h-[380px] sm:min-h-[450px]">
      ...
      >
        <Suspense fallback={null}>
          ...
          />
          <directionalLight position={[-3, 3, -3]} intensity={0.3} color="#e8e4df" />
          <pointLight position={[0, 4, 2]} intensity={0.3} color="#f5f0eb" />
          <MetallicCube />
          <OrbitingSatellites />
          {/* Shadow plane */}
    ...
    <div className="w-full h-full flex items-center justify-center">
      <div className="relative">
        <div
          ...
          style={{
            background: "linear-gradient(135deg, #555 0%, #777 40%, #666 60%, #555 100%)",
            boxShadow: "0 25px 50px rgba(0,0,0,0.12), 0 0 40px rgba(0,0,0,0.04)",
          }}
        />
      </div>
    </div>
